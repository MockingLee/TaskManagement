...
public class AdLoginInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if(request.getSession().getAttribute("loginState").equals(IdentifierString.LOGIN_STATE_LOGIN) && request.getSession().getAttribute("userType").equals(IdentifierString.USER_TYPE_AD)) {
			String userName = request.getSession().getAttribute("loginName").toString();
	        System.out.println("当前管理员已登录，登录的用户名为： " + userName);
	        return true;
		}else {
			response.sendRedirect("redirect:/");
			return false;
